...
watch(
  ...
  (newUser) => {
    if (newUser) {
      ...
      if (!avatarFile.value) {
        avatarPreview.value = newUser.profileImage || '';
      }
    }
  },
...
const onAvatarChange = (e: Event) => {
  ...
  if (target.files?.[0]) {
    ...
    if (file.size > 2 * 1024 * 1024) {
      return toast.warning("รูปภาพต้องมีขนาดไม่เกิน 2MB");
    }
...
<template>
  <div class="max-w-4xl mx-auto py-10 px-4 animate-in fade-in duration-500">
    <div class="bg-white rounded-[3rem] shadow-xl border border-slate-50 overflow-hidden mb-8">
      ...
      <div class="px-10 pb-10 -mt-16 flex flex-col md:flex-row items-end gap-6">
        <div class="relative group">
          <div class="w-32 h-32 rounded-[2.5rem] border-4 border-white bg-slate-100 overflow-hidden shadow-2xl transition-transform group-hover:scale-105 duration-300">
            <img 
              v-if="avatarPreview" 
          ...
          <label class="absolute bottom-0 right-0 p-2 bg-blue-600 text-white rounded-xl shadow-lg cursor-pointer hover:bg-blue-700 transition active:scale-90 z-10">
            <Camera :size="18" />
    ...
    <div class="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-50 min-h-[400px]">
      ...
      <form v-if="activeTab === 'info'" @submit.prevent="handleUpdateProfile" class="space-y-6">
        ...
        <div class="space-y-2">
          ...
          <div class="relative">
            ...
            <input :value="authStore.user?.email" disabled type="email" class="w-full pl-12 pr-4 py-4 bg-slate-100 border-none rounded-2xl text-slate-400 font-bold italic cursor-not-allowed">
          </div>
        </div>
      ...
      <form v-else @submit.prevent="handleChangePassword" class="space-y-6">
        <div class="space-y-2">
          ...
          <div class="relative">
            ...
            <input v-model="passwordForm.currentPassword" type="password" class="w-full bg-slate-50 border-2 border-transparent p-4 rounded-2xl focus:bg-white focus:border-blue-500 outline-none transition-all font-bold">
          </div>
        </div>
        ...
        <div class="space-y-2">
          ...
          <div class="relative">
            <Lock class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" :size="18" />
            <input v-model="passwordForm.newPassword" type="password" class="w-full bg-slate-50 border-2 border-transparent p-4 rounded-2xl focus:bg-white focus:border-blue-500 outline-none transition-all font-bold">
          </div>
        </div>
